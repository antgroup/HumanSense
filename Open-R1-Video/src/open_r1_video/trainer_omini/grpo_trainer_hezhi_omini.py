...
# from transformers.models.qwen2_5_omni.modeling_qwen2_5_omni import Qwen2_5OmniVisionAttention
# # 给类统一加属性，所有实例都会有 is_causal=False
# if not hasattr(Qwen2_5OmniVisionAttention, "is_causal"):
#     Qwen2_5OmniVisionAttention.is_causal = False
...
class Qwen2VLGRPOHezhiTrainer_omini(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            elif "Qwen2.5-Omni" in model_id or "Open-R1-Video" in model_id:
                model_init_kwargs.pop("use_cache")
                model = Qwen2_5OmniForConditionalGeneration.from_pretrained(model, **model_init_kwargs)
        ...
        def is_deepspeed_enabled():
            ...
            This can check whether DeepSpeed is initialized or not.
            """
            try:
                # DeepSpeed has an attribute 'deepspeed' in the model if it's initialized.
                return hasattr(deepspeed, 'initialize')
            except ImportError:
                return False
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                self.ref_model = Qwen2_5OmniForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
                self.ref_model.disable_talker()
            else:
                self.ref_model = AutoModelForCausalLM.from_pretrained(model_id, **model_init_kwargs)
        elif is_deepspeed_enabled():  # DeepSpeed 2.x
            # 处理 DeepSpeed 2.x 下的模型初始化
            if "Qwen2-VL" in model_id:
                self.ref_model = Qwen2VLForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
            elif "Aria" in model_id:
                self.ref_model = AriaForConditionalGeneration.from_pretrained(model_id, **model_init_kwargs)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        data_class = inputs[0]["data_class"]
        print(inputs[0]["if_audio"], inputs[0]["data_class"])
        USE_AUDIO_IN_VIDEO = inputs[0]["if_audio"]
        # USE_AUDIO_IN_VIDEO=True
        prompts_text = self.processing_class.apply_chat_template(inputs[0]["prompt"], add_generation_prompt=True, tokenize=False)
        audios, images, video_inputs = process_mm_info(inputs[0]["prompt"], use_audio_in_video=USE_AUDIO_IN_VIDEO)
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            for i in range(num_generations):  # -1 because we already have one generation
                completion = unwrapped_model.generate(**prompt_inputs, return_audio=False, use_audio_in_video=USE_AUDIO_IN_VIDEO, generation_config=temp_generation_config)
                # completion = unwrapped_model.generate(**prompt_inputs, return_audio=False, use_audio_in_video=USE_AUDIO_IN_VIDEO, thinker_do_sample=True)
                all_completions.append(completion)

            # all_completions = [completion[0] for completion in all_completions]
        ...
        def get_per_token_logps(model, input_ids, **kwargs):
            logits = model.thinker(input_ids, **kwargs).logits  # (B, L, V)
        ...
        # # XXX if input video
        if data_class == "audio":
            prompt_inputs["feature_attention_mask"] = prompt_inputs["feature_attention_mask"].repeat(len(prompt_completion_ids), 1)
            prompt_inputs["input_features"] = prompt_inputs["input_features"].repeat(len(prompt_completion_ids), 1, 1)
        elif data_class == "video_wo_audio":
            ...
            prompt_inputs["video_second_per_grid"] = prompt_inputs["video_second_per_grid"].repeat(len(prompt_completion_ids), 1)
        elif data_class == "video_w_audio":
            prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat(len(prompt_completion_ids), 1)
            prompt_inputs["video_grid_thw"] = prompt_inputs["video_grid_thw"].repeat(len(prompt_completion_ids), 1)
            prompt_inputs["video_second_per_grid"] = prompt_inputs["video_second_per_grid"].repeat(len(prompt_completion_ids), 1)
            prompt_inputs["feature_attention_mask"] = prompt_inputs["feature_attention_mask"].repeat(len(prompt_completion_ids), 1)
